...
public class Parser {
    ...
    /**
     * Are we looking at an IDENTIFIER followed by a COLON? Look ahead to find
     * out.
     * 
     * @return true iff we're looking at IDENTIFIER COLON; false otherwise.
     */

    private boolean seeIdentColon() {
        scanner.recordPosition();
        boolean result = have(IDENTIFIER) && see(COLON);
        scanner.returnToPosition();
        return result;
    }
    
    /**
     * Are we looking at an STATIC followed by a LCURLY? Look ahead to find
     * out.
     * 
     * @return true iff we're looking at STATIC LCURLY; false otherwise.
     */

    private boolean seeStaticLParen() {
        scanner.recordPosition();
    ...
    private boolean seeLocalVariableDeclarationColon() {
        scanner.recordPosition();
        if (have(IDENTIFIER)) {
            // A qualified identifier is ok
            while (have(DOT)) {
    ...
    private JClassDeclaration classDeclaration(ArrayList<String> mods) {
        ...
        Type superClass;
        ArrayList<Type> superInterface = new ArrayList<Type>();
        if (have(EXTENDS)) {
    ...
    private ArrayList<JMember> classBody() {
        ...
        while (!see(RCURLY) && !see(EOF)) {
        	if(seeStaticLParen()){
        		...
        		members.add(new JStaticBlock(line, block()));
        	}
        	else if(see(LCURLY)){
        		line = scanner.token().line();
        		members.add(new JInstanceBlock(line, block()));
        	}
        	else{
    ...
    private JMember interfaceMemberDecl(ArrayList<String> mods) {
        ...
        if (have(VOID)) {
            ...
            ArrayList<JFormalParameter> params = formalParameters();
            JBlock body = null;
            interfaceMemberDecl = new JMethodDeclaration(line, mods, name, type,
                    params, body);
            mustBe(SEMI);
        } else {
            type = type();
     ...
     *               		[expression] SEMI
     *               	[forUpdate] //Basic For
     *               			  | type IDENTIFIER : expression //Enhanced For
     *               			  )
     *               	   RPAREN statement
    ...
    private JStatement statement() {
        ...
        if (see(LCURLY)) {
        	...
        	mustBe(LPAREN);
        	ArrayList<JStatement> forInitStatements = null;
        	Type type = null;
        	ArrayList<JVariableDeclarator> forInitDeclarations = null;
        	JExpression condition = null;
        	ArrayList<JStatement> forUpdate = null;
        	if(seeLocalVariableDeclarationColon()){
        		//Enhanced for loop
        		type = type();
        		mustBe(IDENTIFIER);
        		String name = scanner.previousToken().image();
        		...
        		mustBe(RPAREN);
        		return new JForEnhancedStatement(line, type, name, expression, statement());
        	} else {
        		//Deal with all different cases of For Basic Loop
        		if(have(SEMI)){
        			//No forInit
        			if(have(SEMI)){
        				//No Condition
        			...
        			if(seeLocalVariableDeclaration()){
        				//forInit is type variableDeclarators
        				type = type();
        				forInitDeclarations = variableDeclarators(type);
        				mustBe(SEMI);
        				if(have(SEMI)){
        					//No condition
        					if(see(RPAREN)){
        				...
        				//forInit is statementExpression {COMMA statementExpression}
        				forInitStatements = forInitStatements();
        				mustBe(SEMI);
        				if(have(SEMI)){
        					//No condition
        					if(see(RPAREN)){
        		...
        		mustBe(RPAREN);
        		return new JForBasicStatement(line, forInitStatements,
        				forInitDeclarations, condition, forUpdate, statement());
        	}
        } else if (have(WHILE)) {
            JExpression test = parExpression();
        	...
        	if (have(SEMI)){
        		return new JBreakStatement(line, null);
        		 ...
        		 mustBe(IDENTIFIER);
        	     String labelName = scanner.previousToken().image();
        	     mustBe(SEMI);
        	     return new JBreakStatement(line, labelName);
        	}
        	...
        	if (have(SEMI)){
        		return new JContinueStatement(line, null);
        	     ...
        	     String labelName = scanner.previousToken().image();
        	     mustBe(SEMI);
        	     return new JContinueStatement(line, labelName);
        	}
        } else if (have(SEMI)) {
            return new JEmptyStatement(line);
        } else if (seeIdentColon()){
        	mustBe(IDENTIFIER);
        	String labelName = scanner.previousToken().image();
        	mustBe(COLON);
        	return new JLabelStatement(line, labelName, statement());
        }
    ...
    /**
     * forInitStatement ::= statementExpression {COMMA statementExpression}
     *  @return ArrayList<JStatement>
     */
    private ArrayList<JStatement> forInitStatements() {
    ...
    private JExpression conditionalExpression() {
        ...
        if(have(QUESTION)){
        	JExpression rhsYes = assignmentExpression();
        	mustBe(COLON);
        	lhs = new JConditionalOperator(line, lhs, rhsYes, conditionalExpression());
        }
		return lhs;
	}
    ...
    /**
     * Parse a bitwiseOr expression.
     * 
     * <pre>
     * 	 bitwiseOrExpression ::= bitwiseExclusiveOrExpression // level 9
     *  				  			{BOR bitwiseExclusiveOrExpression}
     * </pre>
     * 
     * @return an AST for a bitwiseOr Expression.
     */

    private JExpression bitwiseOrExpression() {
        int line = scanner.token().line();
        boolean more = true;
        JExpression lhs = bitwiseExclusiveOrExpression();
        while (more) {
            if (have(BOR)) {
                lhs = new JBitwiseOrOp(line, lhs, bitwiseExclusiveOrExpression());
            } else {
                more = false;
            }
        }
    ...
    /**
     * Parse a bitwiseExclusiveOr expression.
     * 
     * <pre>
     *   bitwiseExclusiveOrExpression ::= bitwiseAndExpression // level 8
	 *										{BXOR bitwiseAndExpression}
     * </pre>
     * 
     * @return an AST for a bitwiseExclusiveOr Expression.
     */
    ...
    private JExpression bitwiseExclusiveOrExpression() {
        ...
        while (more) {
            if (have(BXOR)) {
                lhs = new JBitwiseExclusiveOrOp(line, lhs, bitwiseAndExpression());
            } else {
                more = false;
            }
        }
     ...
     * </pre>
     * 
     * @return an AST for a bitwiseAnd Expression.
     */
